...
import { CourierHandoffConflictError, CourierHandoffContractError, CourierHandoffNoLongerCurrentError, parseCourierCustody, parseCourierCustodyRead, parseCourierPickup, projectCourierHandoff } from '../domain/courier-handoff-status.ts';
...
test('assignment loss between sequential reads is never interpreted as missing Custody', async () => {
  const replaced = new CourierHandoffStatusService(async (path) => {
    if (path.endsWith('/custody')) throw new PublicApiError('not_found', 404);
