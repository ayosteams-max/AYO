...
export function CourierStartTravelCommandInfrastructureProvider({ children, identity }: PropsWithChildren<{ identity: TrustedIdentityCommandRuntime }>) {
  ...
  const markArrivedEvidence = useMemo<MarkArrivedEvidenceIntegration>(() => Object.freeze({
    publishFresh: (pickupId, snapshot) => markArrivedScope.publishFresh(pickupId, snapshot),
    clearFresh: (pickupId) => markArrivedScope.clearFresh(pickupId),
  }), [markArrivedScope]);
  return <CapabilityContext.Provider value={capability}><EvidenceContext.Provider value={evidence}><MarkArrivedEvidenceContext.Provider value={markArrivedEvidence}>{children}</MarkArrivedEvidenceContext.Provider></EvidenceContext.Provider></CapabilityContext.Provider>;
}
