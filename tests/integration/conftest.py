...
@pytest.fixture
def migration_built_schema(postgres_engine, clean_postgres_tables):
    """Provide the authoritative migration-built schema, then restore test metadata."""
    with postgres_engine.begin() as connection:
        connection.execute(text(f'DROP SCHEMA IF EXISTS "{AYO_SCHEMA}" CASCADE'))
        connection.execute(text('DROP TABLE IF EXISTS public."ayo_schema_version"'))
        connection.execute(
            text(
                "DO $$ BEGIN "
                "IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='ayo_runtime') "
