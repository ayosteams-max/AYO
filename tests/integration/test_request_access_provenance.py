...
def _principal(engine, *, reuse_seeded_permissions: bool = False) -> tuple[UUID, UUID]:
...
def _configured(
    engine,
    *,
    reuse_seeded_permissions: bool = False,
) -> tuple[RequestAccessApplicationService, UUID, UUID, UUID]:
    service = RequestAccessApplicationService(engine)
    account_id, subject_id = _principal(
        engine, reuse_seeded_permissions=reuse_seeded_permissions
    )
...
def test_provenance_and_continuity_are_database_immutable(
    postgres_engine, migration_built_schema
) -> None:
        ...
        triggers = set(
            connection.execute(
                text(
                    ...
                    "WHERE tgname IN "
                    "('trg_request_access_continuity_immutable', "
                    "'trg_request_access_provenance_immutable') "
    ...
    service, account_id, subject_id, adapter_id = _configured(
        postgres_engine, reuse_seeded_permissions=True
    )
    with pytest.raises(DBAPIError), postgres_engine.begin() as connection:
        connection.execute(
            insert(permissions).values(
                permission_id=uuid4(),
                code="access.provenance.manage",
                description="duplicate must remain rejected",
                created_at=NOW,
            )
        )
    with postgres_engine.connect() as connection:
        seeded_permission_ids = connection.execute(
            select(permissions.c.permission_id).where(
    ...
    assert len(seeded_permission_ids) == 3
    target_id = str(uuid4())
    record = service.record_accepted_interaction(
        actor_account_id=account_id,
        envelope=_envelope(
    ...
    with postgres_engine.connect() as connection:
        stored_target_version = connection.execute(
            select(request_access_interaction_provenance.c.target_version).where(
                request_access_interaction_provenance.c.provenance_id
                == record.provenance_id
            )
        ).scalar_one()
        stored_continuity = connection.execute(
            select(request_access_continuity_references.c.continuity_id).where(
                request_access_continuity_references.c.continuity_id
                == issued.continuity_id
            )
        ).scalar_one()
    assert stored_target_version == 1
    assert stored_continuity == issued.continuity_id
