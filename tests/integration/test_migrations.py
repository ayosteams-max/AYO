...
def _is_governed_courier_pickup_metadata_difference(difference: tuple) -> bool:
    ...
    if kind in {"remove_constraint", "add_constraint"}:
        ...
        return (
            constraint.table.schema == AYO_SCHEMA
            and constraint.table.name == "commerce_courier_pickups"
            and tuple(constraint.columns.keys()) == ("assignment_id",)
            and constraint.name
            in {
...
def _governed_constraint_name(engine, *, table_name: str, logical_name: str) -> str:
    naming_metadata = MetaData(naming_convention=NAMING_CONVENTION)
    table = Table(table_name, naming_metadata, Column("state", String(40)))
    ...
    with postgres_engine.connect() as connection:
        ...
        state_constraints = {
            ...
            for constraint in inspect(connection).get_check_constraints(
                "commerce_courier_dispatch_requests", schema=AYO_SCHEMA
            )
        }
        governed_name = _governed_constraint_name(
            postgres_engine,
            table_name="commerce_courier_dispatch_requests",
            logical_name="courier_dispatch_state_valid",
        )
        assert governed_name in state_constraints
        state_constraint = state_constraints[governed_name]
        assert "missing-governed-constraint" not in state_constraints
