...
8. When assignment authority is removed and the courier status disappears, prior Pickup/Custody evidence is no longer current authority.
...
| Scenario | Synthetic system condition and visible app state/copy | Expected understanding | Existing control or facilitator-only response | Dangerous misunderstanding | Facilitator stop condition |
|---|---|---|---|---|---|
...
| Travelling | Current assignment; **Travelling to the merchant** | Courier is travelling; no arrival or receipt is proven | Remain in the synthetic scenario or use Refresh to re-check | Claims arrival or possession | Would act as though goods were received |
| At merchant | Current assignment; **At the merchant** | Arrival is shown; handoff and possession are not proven | Use Refresh or pause the synthetic scenario; do not infer progress while evidence is unchanged | Claims handoff completed | Treats arrival as transfer |
...
| Ready for handoff | Sealed/ready evidence; **Ready for handoff**; application guidance: **The handoff is ready. Follow the approved in-person process.** | The app refers to a separately approved in-person process; transfer is not complete, and this protocol does not define or approve that process | Use Refresh or pause the synthetic scenario; do not simulate the in-person process | Claims custody already transferred | Treats readiness as possession or attempts a physical handoff |
...
| Assignment no longer current | Previously authorized courier context; a subsequent authoritative read denies current assignment, so mobile removes courier context, status, and locator; another authorized area appears, or **No available area** is shown | Prior Pickup/Custody evidence is no longer authority; disappearance or navigation does not prove completion, a cancellation reason, replacement identity, payment, or delivery | Use another independently authorized area if shown; otherwise use Account or Sign out; facilitator may pause or stop the scenario | Continues from remembered status, treats navigation as completion, assumes hidden work, reuses the locator, or invents a reason for authority loss | Intends to act on removed authority or treats disappearance as proof of completion |
