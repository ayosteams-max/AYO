...
## Exact-blob formatting validation exception

The corrected migration blob predates the current Ruff formatter result and is
already governed as an exact immutable historical artifact. Batch 0 therefore
applies one validation exception to Ruff **format checking only**:

```text
path: database/migrations/versions/20260716_0016_canonical_ride_request.py
blob: 3c1e4b8400567b154582ffbb5f7426d933db1d23
authority: APPROVED CONTROLLED HISTORICAL CORRECTION — PRE-PRODUCTION ONLY
```

The exception is valid only when a fail-closed validator proves all of the
following:

1. the exception allowlist contains exactly the single path above;
2. the path hashes to the exact approved blob above;
3. this governance record contains the approved one-time authority, path and hash;
4. the complete Batch 0 candidate contains only its four approved manifest paths;
5. Ruff lint passes for the migration; and
6. every other Python file, later migration and future change remains subject to
   normal Ruff formatting.

Path relocation, blob modification, a second exempted path, missing authority or
an expanded Batch 0 candidate invalidates the exception. No repository-wide Ruff
configuration, formatter exclusion or future-file exemption is authorized.
