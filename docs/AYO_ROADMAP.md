...
**Mobile Booking intent-foundation checkpoint:** A bounded PRE-PRODUCTION mobile domain, descriptor-safe exact request/response validation, authenticated place/preview transport, single-intent preview engine, in-memory store and unwired non-visual context preserve one cryptographically generated, validated, session-bound preview identity without duplicating Booking authority. The provider constructs the engine around authenticated opaque continuity and exposes no controller, continuity, state producer or store; the importable pure engine alone is not authenticated authority. Deeply frozen request identity, immediate collision rejection and scope-owned single-flight finalization cover synchronous and asynchronous failure without unbounded identifier history. Success atomically stops at `confirmation_locked`; production source contains no confirmation/recovery endpoint, parser, capability, state or outcome-unknown reconciliation. Identity replacement and cancellation erase evidence and suppress late completion. A future separately authorized confirmation/recovery increment must introduce its own identifiers and strict contracts after policy-presentation, legal/product and native-language approval. No screen, durable device store or process-death recovery claim, policy prose, production registry, provider, payment, notification, analytics, dependency, workflow, deployment or activation is included. See `AYO_MOBILE_BOOKING_INTENT_FOUNDATION_ARCHITECTURE.md`.

**Booking consent written-decision and delivery sequence:** The next Booking path is gated in this order: (1) Ethiopia written-decision-cycle architecture; (2) interim confidential-evidence handling authorization; (3) counsel selection and engagement authorization; (4) attributable written counsel answers; (5) counsel-led regulator questions where required; (6) Privacy decision cycle; (7) required-language, native/legal-equivalence and accessibility decisions; (8) verified Ethiopia jurisdiction profile; (9) separately authorized passive authenticated consent-document delivery implementation using disjoint downgrade-resistant schemas, closed Ethiopian Immediate Standard scope, exact preview-bound locale/rendition/domain-separated hashes, continuity-isolated caching and fail-closed withdrawal; and (10) later mobile presentation, dependency review/remediation before normal UI composition, durable persistence and confirmation/recovery architecture and execution, UI/navigation integration, deployment and separate production gates. The approval pack and written-decision template record no completed approval. They introduce no policy prose, endpoint, schema, migration, source, test, dependency, workflow, content activation or production capability. See `AYO_BOOKING_CONSENT_DOCUMENT_DELIVERY_CONTRACT_ARCHITECTURE.md`, `AYO_BOOKING_CONSENT_APPROVAL_PACK_ARCHITECTURE.md`, `AYO_BOOKING_CONSENT_APPROVAL_PACK_TEMPLATE.md`, `AYO_BOOKING_CONSENT_ETHIOPIA_WRITTEN_DECISION_CYCLE_ARCHITECTURE.md` and `AYO_BOOKING_CONSENT_ETHIOPIA_COUNSEL_QUESTION_PACK_TEMPLATE.md`.

Before any Founder document-upload capability, a separate Founder Evidence Intake and Intelligence Board Architecture must govern the phone-first, laptop-optional intake and meeting-preparation experience, evidence custody, original/derived-data separation, Privacy, Security, accessibility, jurisdiction verification and human authority. This reservation changes neither the ten-stage Ethiopia consent sequence nor any current implementation or production authority.
