...
- **Decision:** Propose a separate authenticated, rider-owned, immutable document read rather than embedding content in route-preview responses. A versioned future preview contract selects one exact approved locale/rendition and binds policy family/version, document and rendition identities, format, effective interval, manifest hash, rendition hash and acknowledgment requirement. The existing V1 `content_hash` retains its current legacy semantics and cannot be reinterpreted as localized-rendition evidence. Presentation of V1 metadata fails closed. The initial proposed content format is bounded canonical UTF-8 passive JSON containing plain text only; arbitrary HTML, Markdown, scripts, remote resources, runtime translation and silent language fallback are prohibited.
- **Authority and operation:** Server Booking owns registry selection, current status, rotation and withdrawal. Authenticated identity supplies rider ownership; mobile supplies no rider, policy, document or hash authority. Initial rotation remains immediate mandatory with no grace period. A cached rendition cannot authorize offline acknowledgment because withdrawal cannot be proven offline. Policy approval and software deployment are separated, and exact content changes invalidate downstream approvals. The proposed promotion lifecycle requires legal, product, native-language, accessibility and CTO review followed by attributable Founder approval from Ibrahim Hambentu Shibiru before scheduling or activation; none has occurred.
