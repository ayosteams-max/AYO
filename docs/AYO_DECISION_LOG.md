...
### AP-078 — Proposed Ethiopia Booking consent written decision cycle

- **Date:** 2026-08-16
- **Status:** PROPOSED / PRE-PRODUCTION for CTO review and later Founder/CEO decision. The Ethiopia authority research is complete for question preparation only. No counsel, regulator, ministry, translator, accessibility reviewer, vendor or other external person was contacted; no answer, opinion, submission or approval is recorded.
- **Problem and benefit:** AP-077 requires attributable written external decisions before consent-document implementation, but ungoverned outreach could use the wrong authority, solicit confidential material into an unsafe channel, combine unrelated legal domains or treat receipt, silence or verbal guidance as approval. A closed Ethiopia-only written-decision cycle makes the minimum questions meeting-ready while preserving authenticity, scope, custody, role separation and fail-closed application.
- **Decision:** Propose a written-decision-cycle architecture and unfilled counsel question pack. The cycle separates lead and specialist counsel, counsel and regulator authority, lifecycle state and approval result, public research and confidential evidence, and verification from decision. Every question and answer binds exact scope, source, authority, evidence identity, custody, conditions, expiry and downstream gate. Missing, conflicting, expired, unofficial, unverifiable, out-of-scope or non-responsive evidence remains `BLOCKED`. The authoritative evidence store remains `UNRESOLVED`; sensitive evidence is prohibited from GitHub, AI chat, issue trackers, mobile clients, ordinary email attachments and unmanaged local folders until a separately approved channel and store exist.
- **Authority and red lines:** AI prepares; authorized humans decide; AYO independently verifies. Counsel cannot replace regulator, Privacy, language, accessibility, Product, Security/CTO, independent audit or Founder authority. Founder identity remains label-only as Ibrahim Hambentu Shibiru, Founder & CEO. No approval can override permanent ownership, IP, key, evidence-control, export, audit, unilateral-policy, gate-bypass or production RED conditions.
- **Boundary and sequence:** No private lawyer or evidence vendor is selected. No legal opinion, regulator response, policy wording, retention duration, worldwide authority, implementation, deployment or production authority is created. Next stages are interim confidential-evidence handling authorization, counsel selection/engagement authorization, written counsel answers, counsel-led regulator questions where required, Privacy, language/accessibility and verified Ethiopia-profile decisions, then separately authorized passive implementation and later product/deployment/production gates. The future Founder workflow is phone-first with laptop use optional, but camera/file intake, OCR, translation, storage and Intelligence Board behavior are deferred to a separate Founder Evidence Intake and Intelligence Board Architecture; original bytes remain distinct from derived aids, and no automated output grants authority. See `docs/AYO_BOOKING_CONSENT_ETHIOPIA_WRITTEN_DECISION_CYCLE_ARCHITECTURE.md` and `docs/AYO_BOOKING_CONSENT_ETHIOPIA_COUNSEL_QUESTION_PACK_TEMPLATE.md`.

### AP-079 — External immutable admission record for Ethiopia written-cycle evidence

- **Date:** 2026-08-18
- **Status:** CTO reviewed the architecture and Ibrahim Hambentu Shibiru, Founder & CEO, approved bounded PRE-PRODUCTION implementation only. No candidate, evidence commitment, admission record, signature, trusted key, `ADMITTED` result, deployment or production activation is approved.
- **Problem:** Embedding the reviewed evidence commitment in the workflow made admission self-referential because changing that commitment changed the candidate scanned by `detect-secrets`, which could change substantive scanner evidence and its normalized commitment.
- **Decision:** Separate immutable captured candidate `C` from direct-child admission-only commit `A`. Scanner and assembler always target exact Git commit `C`; active verification executes from exact `A`, loads one canonical repository admission record by Git object identity, requires `A^ == C`, and requires the `C..A` diff to contain only that record. Existing evidence normalization remains limited to the single `candidate_sha` field. No scanner exclusion, moving ref, mutable external authority, automatic approval or AI-invented approval is permitted.
- **Signature and boundary:** The embedded commitment ceases to be executable admission authority. Capture may produce `CAPTURE_ONLY`; active admission remains fail-closed as `ADMISSION_SIGNATURE_AUTHORITY_PENDING` until a separately approved repository-trusted cryptographic signing authority exists. This decision is PRE-PRODUCTION only and does not authorize a real record, Founder/CTO signature, trusted key, commit, push, workflow dispatch, upload, deliberate stop or GitHub operation.

### AP-080 — Admission Schema V2 and fail-closed Founder signature-verification scaffold

- **Date:** 2026-08-18
- **Status:** Founder/CTO approved bounded PRE-PRODUCTION implementation of Admission Schema V2 and the fail-closed Git/OpenSSH verification architecture only.
- **Decision:** Replace the ambiguous `cto_approver` field with immutable `technical_validation_checkpoint` and `technical_review_record_blob` bindings. Bind Founder approval to machine principal `ayo-founder-ibrahim-hambentu-shibiru-v1` and display identity `Ibrahim Hambentu Shibiru, Founder & CEO`. Future public trust authority is repository-owned at `.github/ayo-admissions/trust/founder-admission-signers-v1.txt`, loaded only from captured candidate `C`, and verified with replacement objects and ambient Git signing configuration disabled.
- **Boundary:** No public or private signing key, signature, PIN interaction, admission candidate, evidence commitment, admission record, `ADMITTED` result, deployment or production activation is approved or created. Active admission remains impossible until a separately authorized Founder-attended hardware-key ceremony, public-key enrollment, capture and explicit approval of an exact candidate/evidence tuple.

### AP-081 — Canonical two-key Founder admission trust schema

- **Date:** 2026-08-18
- **Status:** Founder/CTO approved bounded PRE-PRODUCTION implementation only. No key, admission, signature, deployment or production activation is approved.
- **Decision:** Replace the insufficient single-record trust schema with exactly two canonical, independently validated records in fixed PRIMARY-then-RECOVERY order. PRIMARY may be `ACTIVE` or `REVOKED` and is the sole ordinary admission signer. RECOVERY may be `STANDBY` or `REVOKED`; it is never included in ordinary allowed-signers authority, never attempted as fallback and has no executable activation path. Duplicate roles, identifiers, keys and fingerprints fail closed.
- **Boundary:** The real trust-root file remains absent pending the Founder-attended hardware ceremony. Future recovery activation requires a separately reviewed trust-root transition and protocol. This decision creates no public or private key, PIN interaction, admission record, `ADMITTED` result, Git or GitHub operation.
