...
`BACKEND/main.py` registers only the legacy ride, driver-offer, ride-status and wallet routers. Pydantic models validate a small set of requests. `ride_service.py` uses Haversine distance to rank three hard-coded drivers. Mission 12 added an isolated typed immediate-dispatch domain core with server-authoritative creation, idempotency, deterministic explainable scoring, exclusive offers, timeout/reassignment, recovery and audit contracts. Mission 13 adds PostgreSQL 17 tables, transactional dispatch repositories, immutable shared audit writes, a transactional outbox, authenticated/RBAC router contracts and a bounded recovery-worker entry point. The secure dispatch router and worker are intentionally not registered in `BACKEND/main.py`; authentication key/resolver configuration, scheduling, deployment and public activation remain gated. The in-memory dispatch repository is test-only and the unsafe legacy `rides` path is not authoritative dispatch storage. `ayo_ai.py` remains disconnected from dispatch.
